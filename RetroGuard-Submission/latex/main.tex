...
\fancyhead[R]{\thepage}
\fancyfoot[C]{Madhav Dogra \& Aaditya Gaur | RetroGuard: AI-Powered Retroreflectivity Assessment}
...
\author{
    \textbf{Madhav Dogra} \quad \textbf{Aaditya Gaur}\\
    6th NHAI Innovation Hackathon 2026\\
    \texttt{madhavdogra@gmail.com} \quad \texttt{aaditya2605@gmail.com}
}
...
\caption{RetroGuard Digital Twin Dashboard showing the operations overview: hero KPIs (total assets, alerts cleared, average $R_L$), live signal aggregating daily reads, calendar of upcoming compliance reviews and crew dispatches, and the weekly-workload heatmap across corridors. Sidebar navigation exposes the corridor map, asset registry, alerts queue, field capture, forecast, and compliance report.}
\label{fig:dashboard}
\end{figure}
...
\subsection{Working Prototype --- Screenshots}

The figures below capture the current state of the RetroGuard web command
centre running against a seeded dataset of 200 assets across five national
corridors (NH-48, NH-44, NH-27, NH-66, and the Delhi--Meerut Expressway).
The dashboard itself is shown as Fig.~\ref{fig:dashboard} in
Section~\ref{sec:layer5}.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{corridor-map.jpg}
\caption{Corridor map (\texttt{/map}). Each marker is a monitored asset
colour-coded by current $R_L$ health (green = compliant, amber = warning,
red = critical). The right rail shows per-corridor health percentages and a
critical-spotlight list of the worst three assets. Basemap can be toggled
between street, satellite, and terrain layers.}
\label{fig:screen-map}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{asset-registry.jpg}
\caption{Asset registry (\texttt{/assets}). Filterable, sortable,
paginated view of every monitored sign and marking. CSV import/export and
``Add asset'' flows are wired to the FastAPI backend.}
\label{fig:screen-assets}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{alerts-queue.jpg}
\caption{Live alerts queue (\texttt{/alerts}). Alerts are generated
automatically whenever a measurement drops an asset below IRC~67 / IRC~35
thresholds. Severity cards double as filters; ``Resolve'' is an optimistic
update against \texttt{PUT /api/alerts/:id/resolve}.}
\label{fig:screen-alerts}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{measure-calibration.jpg}
\caption{Field capture --- Step 1: calibration against the white
reference patch (\texttt{/measure}). The app translates the camera's
response to the known patch into the per-session brightness-to-$R_L$
coefficient used for the live measurement.}
\label{fig:screen-measure-calibration}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{measure-capture.jpg}
\caption{Field capture --- Step 3: capture and result (\texttt{/measure}).
A measurement of $R_L=125.3$ on an NH-48 thermoplastic road marking
(IRC min $=150$) flips the asset status to \textbf{critical}; a live alert
is automatically enqueued on the \texttt{/alerts} page.}
\label{fig:screen-measure-capture}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{compliance-report.jpg}
\caption{Compliance report (\texttt{/reports}). Per-corridor audit view
with compliance rate, status breakdown, and per-asset-type compliance
percentages. Period selector and PDF export are wired; critical assets
requiring immediate attention are surfaced at the bottom.}
\label{fig:screen-report}
\end{figure}
