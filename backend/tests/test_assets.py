...
def test_create_asset_roundtrip(client):
...
def test_update_asset(client):
    ...
    r2 = client.put(f"/api/assets/{asset_id}", json={"material_grade": "diamond", "orientation": "right"})
    assert r2.status_code == 200
    updated = r2.json()
...
def test_csv_import_happy_path(client):
    r = client.post("/api/assets/import", files=_csv_file(_CSV_3_ROWS))
...
def test_csv_import_with_duplicates(client):
...
def test_csv_import_with_malformed_row(client):
    bad_csv = (
        _CSV_HEADER
        + "sign,NH-CSV,bad_chainage,28.6,77.2,high_intensity,2022-01-01,left,250\n"
    )
    r = client.post("/api/assets/import", files=_csv_file(bad_csv))
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["created"] == 0
...
def test_csv_import_force(client):
    ...
    r1 = client.post("/api/assets/import", files=_csv_file(unique_csv))
    ...
    r2 = client.post("/api/assets/import/force", json=force_payload)
